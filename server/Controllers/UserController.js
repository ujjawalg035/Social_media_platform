...
export const getUserFollowers = async (req, res) => {
    ...
    try {
        ...
        const followersWithoutPassword = followers.map((follower) => {
            const { password, ...otherDetails } = follower._doc;
            return otherDetails;
        });

        res.status(200).json(followersWithoutPassword);
    } catch (error) {
        res.status(500).json(error);
    }
}
...
export const getUserFollowing = async (req, res) => {
    ...
    try {
        ...
        const followingWithoutPassword = following.map((followedUser) => {
            const { password, ...otherDetails } = followedUser._doc;
            return otherDetails;
        });

        res.status(200).json(followingWithoutPassword);
    } catch (error) {
        res.status(500).json(error);
    }
}
